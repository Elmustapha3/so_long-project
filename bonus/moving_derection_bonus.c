...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   moving_derection_bonus.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: eej-jama <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/01 13:26:42 by eej-jama          #+#    #+#             */
/*   Updated: 2023/03/08 16:57:15 by eej-jama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	up(t_mlx_vars *m_v, int num_col, int *printer, t_point p)
{
	...
	if (m_v->map[p.x - 1][p.y] != '1' && m_v->map[p.x - 1][p.y] != 'E')
	{
		...
		mlx_put_image_to_window(m_v->mlx, m_v->mlx_win, m_v->img.spc, 50 * p.y,
			50 * p.x);
		mlx_put_image_to_window(m_v->mlx, m_v->mlx_win, m_v->img.wll, 50 * 14,
			0);
		mlx_string_put(m_v->mlx, m_v->mlx_win, 50 * 14, 18, 0xFFFFFF,
			ft_itoa(++(*printer)));
	}
	return (num_col);
}
